...
public class Either<L, R> {
    ...
    public static <L, R> Either<L, List<R>> sequence(List<Either<L, R>> eitherList, BinaryOperator<L> accumulator) {
        if (eitherList.stream().allMatch(Either::isRight)) {
            ...
            // 1.取第一个异常
//            return left(eitherList.stream().filter(Either::isLeft).findFirst().orElseThrow(new RuntimeException("异常获取失败")).getLeft());
            // 2.取全部异常
            return left(eitherList
                    ...
                    .map(Either::getLeft)
                    .reduce(accumulator)
                    .orElseThrow(() -> new RuntimeException("异常获取失败"))
            );
        }
    }
